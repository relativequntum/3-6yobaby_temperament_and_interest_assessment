...
**报告大纲（章节顺序，render.py 已按此装配，叙事按对应 narrative key 写）**：
① 一页速览 → ② 概览（精简）→ ③《怎么读这份报告》（**render 层静态段，你不用写**：测什么 + 为什么只和自己比·非诊断 + 名词解释 + 怎么看图 + 建议来自循证活动库）→ ④ 行动建议（**已上移到此**）→ ⑤ 详细分析（气质 / 社会情绪与自我调节 / 发展快照 / 兴趣 / 学习品质 / 家庭与环境）→ ⑥ 综合画像 → ⑦ 边界声明 + 软提示 + 复测 → ⑧ 附录（建议来源与依据 + 方法学与局限）。

> ③《怎么读这份报告》与⑧附录的来源表是 render.py 固定生成的静态内容（措辞统一、可控），你**不需要也不应**在叙事里重写这两段；只需照常写各章 narrative 文本。

**各章叙事强约束**（可微调措辞，但要点都要在）：
1. 开篇致家长（overview）：用昵称、暖场、点明「这是优势与兴趣画像，不是考试、不是诊断」。
2. 气质小像（temperament）：**开头必须用大白话解释「气质是什么」并标英文 `气质（Temperament）`**——「天生、相对稳定的行事风格（好动/安静、慢热/自来熟、情绪快/慢），没有好坏，是做事的底色」；再用 `raw_band` 锚点语义 + 个体内相对位描述，去刻板化。
...
6. 给家长的活动建议：从候选 shortlist **裁剪**，每条仍需挂真实 activity id（写进 RCO 的 activity，供附录溯源——**正文不再显示编号与证据标签**）；按 `temperament_fit_notes` 因娃调适；三类（游戏/锻炼/陪伴）均衡。
7. 温和收尾 + 非诊断免责声明 + （若有）红旗软提示。

**全篇措辞规范（人话）**：
- **专业构念名首次出现一律标英文原文**，如 `努力控制（Effortful Control）`、`个体内（ipsative）`、`气质（Temperament）`、`学习品质（approaches to learning）`。
- 各段凡涉及「子维度题少 / 相互相关高 / 不参与排序」的免责，统一用大白话：**「下面这些更细的小项，每项只问了一两道题、不够稳，意思上也有重叠，所以只供你参考、不拿来给孩子排强弱——排名只用题目够多够稳的大块。」**（render 层的静态子维度脚注已是此文案；你叙事里若提及也照此口径，不用术语。）
- 建议来源说明已由 render 固定放进③《怎么读》末与⑧附录，你不必在正文逐条标「有据/一般性发展原理」。
